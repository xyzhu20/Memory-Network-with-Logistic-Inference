...
class MRCModel(pretrained_model_class):
    def __init__(self, config):
        ...
        self.verifier = nn.Linear(config.hidden_size, 1)  # gate verifier on squad
        ...
        self.QA_coattn = CoAttention(head=head_num, input_dim=config.hidden_size, dropout=0.4)
        self.gate_coattn = CoAttention(head=head_num, input_dim=config.hidden_size, dropout=0.4)
        ...
        self.internal_fusion = nn.Sequential(
            nn.Linear(config.hidden_size * 4, config.hidden_size),
            nn.Tanh()
        )
    ...
    def forward(
        ...
        speaker_loss_fct = nn.BCEWithLogitsLoss() if args.model_type == 'electra' else nn.BCEWithLogitsLoss(
            reduction='sum')
        utter_loss_fct = CrossEntropyLoss(ignore_index=128)

        # deal with utterance information
        utter_logits, utter_weights, utter_weights_repeated = self.key_utterance_prediction(hidden_states,
                                                                                            utterance_gather_ids,
                                                                                            utterance_p_mask,
                                                                                            utterance_repeat_num,
                                                                                            attention_mask)

        speaker_logits, speaker_mha_out, speaker_attentions = self.speaker_prediction(hidden_states,
        ...
        coattention_QA = self.utterance_interaction(fused_hidden_states, utterance_gather_ids, attention_mask, utter_weights)
        ...
        if training:
            ...
            speaker_loss = speaker_loss_fct(speaker_logits,
                                            speaker_target) * 10 if args.model_type == 'electra' else speaker_loss_fct(
                speaker_logits, speaker_target) / bsz
            ...
            answer_list = []
            utterance_indices = torch.tensor([i for i in range(128 if not args.draw else 16)]).unsqueeze(0).expand(bsz,
                                                                                                                   -1)  # (bsz, utter_num)
            utteracne_id_expand = utterance_indices.reshape(-1).repeat_interleave(
                utterance_repeat_num.view(-1).cpu()).view(bsz, -1)  # (bsz, slen)
            ...
            for bidx in range(bsz):
                ...
                answer_list.append((qid[bidx], \
                                    {"answer_text": best_text, "prob": best_prob, "span_pred_uid": span_pred_uid,
                                     "model_pred_uid": model_pred_uid}))

        outputs = (total_loss, span_loss, utter_loss, speaker_loss) if training else [answer_list,
                                                                                      (correct_num, all_num)]
    ...
    def speaker_prediction(self, hidden_states, speaker_hidden_states, speaker_attn_mask, speaker_gather_ids,
                           target_speaker_gather_id, speaker_target_mask, attention_mask, output_attentions,
                           training):
        bsz, slen, hsz = hidden_states.size()
        ...
        speaker_attn_mask = speaker_attn_mask.unsqueeze(1).unsqueeze(1).expand(-1, self.config.num_attention_heads,
                                                                               slen, -1)  # (bsz, n_heads, slen, slen)
        attn_mask_expand = attention_mask
        attn_mask_expand[:, self.question_start:] = 0
        attn_mask_expand = (1 - attn_mask_expand) * -1e30
        attn_mask_expand = attn_mask_expand.unsqueeze(1).unsqueeze(1).expand(-1, self.config.num_attention_heads, slen,
                                                                             -1)

        mha_outs = getattr(self, "MHA_0")(hidden_states_detached,
                                          attention_mask=speaker_attn_mask if training else attn_mask_expand,
                                          output_attentions=output_attentions)
        ...
        for i in range(1, self.speaker_mha_layers):
            mha_outs = getattr(self, "MHA_{}".format(str(i)))(speaker_mha_out,
                                                              speaker_attn_mask if training else attn_mask_expand,
                                                              output_attentions=output_attentions)
        ...
        return speaker_logits, speaker_mha_out, speaker_attentions

    def key_utterance_prediction(self, hidden_states, utterance_gather_ids, utterance_p_mask,
                                 utterance_repeat_num, attention_mask):
        bsz, slen, hsz = hidden_states.size()
        # CoAttention deal with gatelogits
        ...
        coattn1, coattn2 = self.gate_coattn(Passage, Question, mask_P, mask_Q)  # CoAttention Passage and Question

        gate_coattention = self.gate_fusion(
            torch.cat([coattn1, coattn2, coattn1 * coattn2, coattn1 - coattn2], dim=-1)
        )

        # predict key-utterance
        utter_embs = hidden_states.gather(dim=1, index=utterance_gather_ids.unsqueeze(-1).expand(-1, -1,
                                                                                                 hsz))  # (bsz, max_utter, hsz)
        gate_coattention_emb_expand = gate_coattention.unsqueeze(1).expand_as(utter_embs)
        utter_logits = self.utter_filter(torch.cat(
            [utter_embs, gate_coattention_emb_expand, utter_embs * gate_coattention_emb_expand,
             utter_embs - gate_coattention_emb_expand],
            dim=-1)
        ).squeeze(-1)  # (bsz, max_utter)
        ...
        return utter_logits, utter_weights, utter_weights_repeated

    def utterance_interaction(self, hidden_states, utterance_gather_ids, attention_mask, utter_weights):
        bsz, slen, hsz = hidden_states.size()
        ...
        coattn_QA_1, coattn_QA_2 = self.QA_coattn(Passage, QA, mask_P, mask_QA)  # Co-Attention with Passage and QA
    ...
    def train_MemEncoder(
        ...
        # CoAttention deal with gate logits
