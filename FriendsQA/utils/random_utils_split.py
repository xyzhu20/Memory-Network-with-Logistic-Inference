# utils for BERT-base on FriendsQA
        ...
        for dialogue in paragraphs:
            ...
            for qa in dialogue['qas']:
                ...
                if not training: # during inference
                    ...
                    # in line with Liu et. al. (2020) BERT version
                    answer_utter_id = qa['answers'][0]['utterance_id']
...
def convert_examples_to_features(examples, tokenizer, max_length, speaker_mask_path, training=True, max_utterance_num=128):
    ...
    def _select_speaker(speaker_counts: dict, top_k=1):
        sorted_counts = dict(sorted(speaker_counts.items(), key=lambda x: x[1], reverse=True))
        candidate_speakers = list(sorted_counts.keys())[:top_k]
        select_idx = random.randint(0, min(top_k-1, len(sorted_counts)-1))
        selected_speaker = candidate_speakers[select_idx]
    ...
    for fidx, exp in enumerate(tqdm(examples)):
        ...
        # random version
        selected_speaker = _select_speaker(speaker_counts)
        utter_number = speaker_counts[selected_speaker]
        speaker_mask_index = random.randint(1, utter_number)
